...
/** UI-only normalizer — the gateway transmits its own hostname (which on some
 *  hardware embeds the vendor SDK name); we display a neutral label instead.
 *  Internal data is untouched. */
function neutralizeGatewayName(name: string | undefined | null): string {
  if (!name) return 'edge-gateway';
  if (/rdk/i.test(name) || /bpi/i.test(name)) return 'edge-gateway-01';
  return name;
}
              ...
              <tbody>
                ...
                {!liveState && (
                  <tr><td colSpan={6} style={{ color: 'var(--text-muted)', fontSize: 12.5, padding: '14px 0' }}>
                    Waiting for first IPsec telemetry payload from the gateway…
                  </td></tr>
                )}
              </tbody>
            </table>
  ...
  return (
    ...
    >
      ...
      <div style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 12, lineHeight: 1.5 }}>
        All values derive from the live IPsec gateway telemetry protobuf
      ...
      <div style={{
        ...
        {error ? (
          ...
          <div style={{ color: 'var(--text-muted)', display: 'inline-flex', alignItems: 'center', gap: 8 }}>
            ...
            Reading the latest IPsec telemetry payload from the gateway…
          </div>
        ) : (
 ...
 * (gateway → MQTT IPsec metrics topic → protobuf decode in our server → SSE).
...
/** A real sample payload captured from the edge gateway device — used for
...
const SAMPLE_IPSEC_GATEWAY: IpsecGatewayState = {
  ...
  metrics: {
    ...
    gateway: { name: 'edge-gateway-01', mac: '02:01:00:60:53:8b', prim_wan_ip: '192.168.1.201', sec_wan_ip: 'none' },
  },
};
...
function LiveIpsecCard({
  ...
  const pill = showSample
    ...
    : ipsec.connected
      ? <span className="badge ok"><span className="dot ok" />Streaming · live IPsec feed</span>
      : ipsec.lastError
      ...
      right={
        <div className="toolbar">
          <button
            ...
            title="Render the UI using a captured edge-gateway payload"
            style={showSample
              ? { background: 'var(--grad-accent-soft)', borderColor: 'var(--accent)', color: 'var(--text)' }
              : undefined}
          >
        ...
        <div style={{
          ...
          <div>
            <div style={{ color: 'var(--text-dim)', fontWeight: 600 }}>
              No payload yet on the IPsec telemetry feed
            </div>
    ...
    <div className="ipsec-gw">
      ...
      <div className="ipsec-gw-head">
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div className="ipsec-gw-name">{neutralizeGatewayName(m.gateway.name)}</div>
    ...
    <div className="ipsec-flow-wrap">
      <svg viewBox={`0 0 ${W} ${H}`} style={{ width: '100%', height: 'auto', display: 'block' }}>
        ...
        <SysNodeBox x={COL_GW.x} y={ROW_CENTER - GW_H / 2} w={COL_GW.w} h={GW_H}
          ...
          label={neutralizeGatewayName(m.gateway.name)}
