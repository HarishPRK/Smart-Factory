...
const FactoryExtras3D: React.FC = () => {
  ...
  useFrame(({ clock }, delta) => {
    ...
    if (now - lastCounterUpdate.current > 500) {
      ...
      if (energyValueRef.current) {
        ...
        const blowActive =
          stages.find((s) => s.id === "forming")?.status === "running";
    ...
    if (energyNeedleRef.current) {
      ...
      energyNeedleRef.current.rotation.z =
        baseAngle + normalized * Math.PI * 0.8;
    }
  ...
  return (
    <group>
      ...
      {/* Per-stage bottle counters removed for cleaner view */}
      ...
      >
        ...
        >
          ...
          <div
            ...
            style={{ fontSize: "11px", fontWeight: 700, color: "#fbbf24" }}
          >
            Morning (6AM-2PM)
          </div>
        </div>
